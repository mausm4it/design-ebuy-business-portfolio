...
class PortfolioController extends Controller
{
    ...
    public function show($slug)
    {
        ...
        $portfolio_count_categories = Portfolio::select(DB::raw('count(*) as category_count, category_id'))
            ...
            ->where('portfolios.post_type', 'works')
            ->groupBy('category_id')
            ->get();
        $portfolio_count_categories_feature = Portfolio::select(DB::raw('count(*) as category_count, category_id'))
            ->where('language_id', $language->id)
            ->where('portfolios.status', 1)
            ->where('portfolios.post_type', 'feature')
            ->groupBy('category_id')
            ->get();
            
            
        $portfoliolnk= Portfolio::all();
           
        return view('frontend.portfolio.show', compact('site_info', 'google_analytic', 'portfoliolnk',
            'socials', 'breadcrumb', 'external_url', 'quick_access_button', 'recent_posts', 'footer_pages',
            'portfolio', 'details', 'sliders', 'portfolio_count_categories', 'portfolio_count_categories_feature' , 'color_option'));
    }
    ...
    public function category_show($category_name)
    {
     ...
     public function category_show2($category_name)
    {
        // Get site language
        $language = getSiteLanguage();
        ...
        return view('frontend.portfolio.category-show2', compact('site_info',
            'google_analytic', 'socials', 'breadcrumb', 'external_url', 'quick_access_button',
            'footer_pages', 'portfolios', 'category', 'color_option'));
    }
